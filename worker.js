export default {
  async fetch(request, env, ctx) {
    ...
    forwardHeaders.set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36");
    
    // Inject the dynamic referer if provided
    if (referer) {
        ...
        const rewrittenManifest = manifestText
          ...
          .map((line) => {
            ...
            if (trimmed && !trimmed.startsWith("#")) {
              try {
                // Resolves relative chunk paths against the parent URL
                const absoluteUrl = new URL(trimmed, targetUrl).href;
                
                // Rewrite chunk URL to pass through the proxy AND keep the referer
